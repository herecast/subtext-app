...
test('news show page, home page referring', function(assert) {
  ...
  click(
    ...
    testSelector('news-card', news1.title)
  );
  ...
  andThen(()=>{
    ...
    assert.equal(
      find(testSelector('page', 'location.index')).length, 1,
      "Displays location.index page in background");
...
test('event show page, home page referring', function(assert) {
  const location = server.create('location');
  const event1 = server.create('event');
  ...
  click(
    testSelector('link', 'show'),
    testSelector('event-card', event1.id)
  );
...
test('talk show page, home page referring', function(assert) {
  const location = server.create('location');
  const user = server.create('user', {locationId: location.id, email: "embertest@subtext.org"});
  ...
  click(
    testSelector('link', 'show'),
    testSelector('talk-card', talk1.id)
  );

  andThen(()=>{
    assert.ok(
      currentURL().indexOf(`/talk/${talk1.id}`) === 0);
...
test('market show page, home page referring', function(assert) {
  const location = server.create('location');
  const market1 = server.create('market-post');
  ...
  click(
    testSelector('link', 'show'),
    testSelector('market-card', market1.id)
  );
