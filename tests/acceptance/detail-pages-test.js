...
module('Acceptance | detail pages', function(hooks) {
  ...
  test('testing news detail page', async function(assert) {
    ...
    assert.equal($(find('[data-test-news-title]')).text().trim(), content.title, 'it should have the correct title');
  ...
  test('testing talk detail page', async function(assert) {
    ...
    assert.equal($(find('[data-test-market-title]')).text().trim(), content.title, 'it should have the correct title');
  ...
  test('testing market detail page', async function(assert) {
    ...
    assert.ok(find('[data-test-market-thumbnail]'), 'it should show the market thumbnail images');
    assert.equal($(find('[data-test-market-cost]')).text(), content.cost, 'it should show the correct market cost');
    assert.equal($(find('[data-test-market-location]')).text().trim(), [location.city, location.state].join(', '), 'it should show the correct market location');
    assert.equal($(find('[data-test-market-content]')).text().trim().substring(0, 50), content.content.substring(0, 50), 'it should show the detail page content');
    assert.ok(find('[data-test-market-attribution]'), 'it should show the attribution');
    assert.ok(find('[data-test-comments-section]'), 'it should show the comments section');
  });
