...
export default ModalInstance.extend({
  model: null,
  profileContentMetricsModel: null,
  ...
  title: computed('model.title', function() {
    return `Showing data for all ${get(this, 'model.title')} owned content.`;
  }),

  lifetimeViewCount: computed.oneWay('model.viewCount'),
  hasViewData: computed.notEmpty('profileContentMetricsModel.views'),
  sortedViews: computed.sort('profileContentMetricsModel.views', 'sortBy'),
  ...
  _updateStartDate(selectedOption){
    ...
    run.debounce(this, this._getProfileContentMetricsModel, 200);
  },
  ...
  _getProfileContentMetricsModel() {
    if (!get(this, 'isDestroying')) {
      const dateQuery = get(this, '_dateQuery');
      set(this, 'isLoadingModel', true);

      get(this, 'contentMetrics').getMetrics('organization', get(this, 'model.id'), dateQuery)
      .then((profileContentMetricsModel) => {
        this.setProperties({
          profileContentMetricsModel: profileContentMetricsModel,
          isLoadingModel: false
        });
      });
    }
